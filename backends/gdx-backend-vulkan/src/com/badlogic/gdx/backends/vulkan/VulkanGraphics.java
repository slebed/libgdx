...
public class VulkanGraphics extends AbstractGraphics implements Disposable {
	...
	public VulkanGraphics(VulkanWindow window) {
		this.window = window;
		if (window.getConfig().glEmulation == VulkanApplicationConfiguration.GLEmulation.GL32) {
			this.gl20 = this.gl30 = this.gl31 = this.gl32 = new Lwjgl3GL32();
		} else if (window.getConfig().glEmulation == VulkanApplicationConfiguration.GLEmulation.GL31) {
			this.gl20 = this.gl30 = this.gl31 = new Lwjgl3GL31();
		} else if (window.getConfig().glEmulation == VulkanApplicationConfiguration.GLEmulation.GL30) {
			this.gl20 = this.gl30 = new Lwjgl3GL30();
		} else {
			try {
				this.gl20 = window.getConfig().glEmulation == VulkanApplicationConfiguration.GLEmulation.GL20 ? new Lwjgl3GL20()
