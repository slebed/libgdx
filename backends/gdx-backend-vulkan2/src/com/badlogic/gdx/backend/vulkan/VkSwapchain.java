	...
	public static class Builder {
		...
		public VkSwapchain build () {
			...
			try (MemoryStack stack = stackPush()) {
				...
				extent.width(Math.max(surfaceCapabilities.minImageExtent().width(),
					Math.min(surfaceCapabilities.maxImageExtent().width(), width)));
				extent.height(Math.max(surfaceCapabilities.minImageExtent().height(),
					Math.min(surfaceCapabilities.maxImageExtent().height(), height)));
				...
				VkSwapchainCreateInfoKHR createInfo = VkSwapchainCreateInfoKHR.calloc(stack)
					...
					.imageFormat(imageFormat).imageColorSpace(chosenFormat.colorSpace()).imageExtent(extent).imageArrayLayers(1)
					...
					.presentMode(chosenPresentMode).clipped(true).oldSwapchain(VK_NULL_HANDLE);

				VkMemoryUtil.vkCheck(vkCreateSwapchainKHR(device.getRawDevice(), createInfo, null, pSwapchain),
		...
		private long createImageView (long image, int imageFormat) {
			try (MemoryStack stack = stackPush()) {
				VkImageViewCreateInfo viewInfo = VkImageViewCreateInfo.calloc(stack).sType(VK_STRUCTURE_TYPE_IMAGE_VIEW_CREATE_INFO)
					.image(image).viewType(VK_IMAGE_VIEW_TYPE_2D).format(imageFormat).subresourceRange(range -> range
						.aspectMask(VK_IMAGE_ASPECT_COLOR_BIT).baseMipLevel(0).levelCount(1).baseArrayLayer(0).layerCount(1));

				LongBuffer pImageView = stack.mallocLong(1);
				VkMemoryUtil.vkCheck(vkCreateImageView(device.getRawDevice(), viewInfo, null, pImageView),
