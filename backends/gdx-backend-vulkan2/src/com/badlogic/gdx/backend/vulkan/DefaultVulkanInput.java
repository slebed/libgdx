...
public class DefaultVulkanInput extends AbstractInput implements VulkanInput {
    ...
    private GLFWCursorPosCallback cursorPosCallback = new GLFWCursorPosCallback() {
        ...
        public void invoke(long windowHandle, double x, double y) {
            ...
            if (graphics != null && window.getConfig().hdpiMode == HdpiMode.Pixels) {
        ...
        public void invoke(long windowHandle, int button, int action, int mods) { // Use different name than member field 'window'
            try {
                ...
                if (action == GLFW.GLFW_PRESS) {
                    mousePressed++;
                    justTouched = true; // Assuming these fields exist in DefaultVulkanInput
                    ...
                    // eventQueue.keyDown(...); // Optional key mapping
                } else { // action == GLFW.GLFW_RELEASE
                    ...
                    // eventQueue.keyUp(...); // Optional key mapping
                }
            } catch (Throwable t) {
                // Log error (already implemented)
                if (Gdx.app != null) {
                    Gdx.app.error("DefaultVulkanInput", "Error in mouseButtonCallback", t);
                } else {
    ...
    public boolean isButtonPressed(int button) {
        if (button < 0 || button >= isButtonPressed.length) return false; // Bounds check
        return isButtonPressed[button]; // <<< READS the array state
    }
