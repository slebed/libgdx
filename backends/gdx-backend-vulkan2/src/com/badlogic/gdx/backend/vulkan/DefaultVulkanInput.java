...
public class DefaultVulkanInput extends AbstractInput implements VulkanInput {
    private String TAG = "DefaultVulkanInput";
    ...
    private final boolean[] isButtonPressed = new boolean[Input.Buttons.FORWARD + 1]; // Size 5 (Indices 0-4)
    private final boolean[] justPressedButtons = new boolean[Input.Buttons.FORWARD + 1]; // Size 5
    char lastCharacter;
    ...
    private final GLFWCharCallback charCallback = new GLFWCharCallback() {
        ...
        public void invoke(long window, int codepoint) {
            ...
            DefaultVulkanInput.this.window.getGraphics().requestRendering();
    ...
    private final GLFWCursorPosCallback cursorPosCallback = new GLFWCursorPosCallback() {
        ...
        public void invoke(long windowHandle, double x, double y) {
            ...
            Graphics graphics = Gdx.graphics; // Get static instance
            if (graphics != null && window.getApplicationConfig().hdpiMode == HdpiMode.Pixels) {
                float logicalWidth = (float) graphics.getWidth(); // Window/Logical width
                float logicalHeight = (float) graphics.getHeight(); // Window/Logical height
                // Avoid division by zero if window is minimized etc.
                if (logicalWidth > 0 && logicalHeight > 0) {
                    float xScale = graphics.getBackBufferWidth() / logicalWidth; // Backbuffer / Logical
                    float yScale = graphics.getBackBufferHeight() / logicalHeight; // Backbuffer / Logical
            ...
            DefaultVulkanInput.this.window.requestRendering();
    ...
    private final GLFWMouseButtonCallback mouseButtonCallback = new GLFWMouseButtonCallback() {
        @Override
        public void invoke(long windowHandle, int button, int action, int mods) { // Use different name than member field 'window'
            ...
            try {
                ...
                if (action == GLFW.GLFW_PRESS) {
                    mousePressed++;
                    justTouched = true;
                    isButtonPressed[gdxButton] = true;
                    ...
                    isButtonPressed[gdxButton] = false;
                ...
                if (Gdx.app != null) {
                    Gdx.app.error(TAG, "Error in mouseButtonCallback", t);
                } else {
                    System.err.println("Error in mouseButtonCallback:");
                    t.printStackTrace();
                }
            }
        }
    ...
    @Override
    public void update() {
        //InputProcessor currentProcessor = this.inputProcessor;
        //Gdx.app.log(TAG, "Update called. Processor: " + (currentProcessor == null ? "NULL" : currentProcessor.getClass().getName()));
    ...
    public boolean isButtonPressed(int button) {
        if (button < 0 || button >= isButtonPressed.length) return false;
    ...
    public void setCursorPosition(int x, int y) {
        if (window.getApplicationConfig().hdpiMode == HdpiMode.Pixels) {
            float xScale, yScale;
            if (window.getBackBufferWidth() == 0) {
                xScale = 1.0f;
            } else {
                xScale = (float) window.getLogicalWidth() / window.getBackBufferWidth();
            }
            if (window.getBackBufferHeight() == 0) {
                yScale = 1.0f;
            } else {
                yScale = (float) window.getLogicalHeight() / window.getBackBufferHeight();
            }
