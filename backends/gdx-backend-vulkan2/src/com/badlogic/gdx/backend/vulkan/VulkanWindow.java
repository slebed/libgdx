...
public class VulkanWindow implements Disposable {
	...
	VulkanWindow (ApplicationListener listener, Array<LifecycleListener> lifecycleListeners, VulkanApplicationConfiguration config,
		VulkanApplicationBase application) {
	...
	boolean update () {
		...
		/*
		 * if (asyncResized) { asyncResized = false; //Gdx.graphics.updateFramebufferInfo(); //graphics.gl20.glViewport(0, 0,
		 * graphics.getBackBufferWidth(), graphics.getBackBufferHeight()); listener.resize(Gdx.graphics.getWidth(),
		 * Gdx.graphics.getHeight()); //Gdx.graphics.update(); listener.render(); //GLFW.glfwSwapBuffers(windowHandle); return true;
		 * }
		 */
	...
	VulkanApplicationConfiguration getConfig () {
		return config;
	}
