	...
	public int acquireNextImage (long signalSemaphore, long fence, IntBuffer pImageIndex) {
		...
		if (handleToCheck == VK_NULL_HANDLE) {
			Gdx.app.error("VulkanSwapchain",
				"[Acquire] PRE-CHECK FAILED: swapchain handle is VK_NULL_HANDLE. Flagging for recreation.");
			needsRecreation = true;
			return VK_ERROR_INITIALIZATION_FAILED;
		}
		// Gdx.app.log("VulkanSwapchain", "[Acquire] PRE-NATIVE CALL: About to call vkAcquireNextImageKHR with handle: " +
		...
		private static VulkanSwapchain createSwapchainResources (Object source, boolean useVSync, long oldSwapchainHandle) {
			...
			try (MemoryStack stack = stackPush()) {
				...
				Gdx.app.log("VulkanSwapchain.Builder",
					"[CreateResources] vkCreateSwapchainKHR returned valid handle: " + swapchainHandle);
