...
public class VulkanSwapchain implements Disposable {
    ...
    private VulkanSwapchain(Builder builder,
                            long swapchainHandle, List<Long> images, List<Long> imageViews,
                            long renderPassHandle, List<Long> framebuffers,
                            VkExtent2D chosenExtent, int chosenFormat, boolean initialVSync) {
    ...
    public int acquireNextImage(long signalSemaphore, long fence, IntBuffer pImageIndex) {
        ...
        //Gdx.app.log("VulkanSwapchain", "[Acquire] Checking handle: " + handleToCheck); // Log before check
        if (handleToCheck == VK_NULL_HANDLE) {
            Gdx.app.error("VulkanSwapchain", "[Acquire] PRE-CHECK FAILED: swapchain handle is VK_NULL_HANDLE. Flagging for recreation.");
            needsRecreation = true;
            return VK_ERROR_INITIALIZATION_FAILED;
        }
        //Gdx.app.log("VulkanSwapchain", "[Acquire] PRE-NATIVE CALL: About to call vkAcquireNextImageKHR with handle: " + handleToCheck); // Log right before native call
        ...
        if (acquireResult == VK_ERROR_OUT_OF_DATE_KHR || acquireResult == VK_SUBOPTIMAL_KHR) {
            ...
            Gdx.app.log("VulkanSwapchain", "acquireNextImage result: " + VkResultDecoder.decode(acquireResult) + ". Flagged for recreation.");
    ...
    public int present(VkQueue presentQueue, int imageIndex, long waitSemaphore) {
        ...
        try (MemoryStack stack = stackPush()) {
            ...
            if (presentResult == VK_ERROR_OUT_OF_DATE_KHR || presentResult == VK_SUBOPTIMAL_KHR) {
                needsRecreation = true; // Mark for recreation on next frame
                Gdx.app.log("VulkanSwapchain", "vkQueuePresentKHR result: " + VkResultDecoder.decode(presentResult) + ". Flagged for recreation.");
            } else if (presentResult != VK_SUCCESS) {
                Gdx.app.error("VulkanSwapchain", "Failed to present swap chain image! Result: " + VkResultDecoder.decode(presentResult));
     ...
     * IMPORTANT: Assumes vkDeviceWaitIdle() has been called externally before calling this method.
     */
    ...
    public static class Builder {
        ...
        private void validate() {
            ...
            Objects.requireNonNull(device.getPhysicalDevice(), "PhysicalDevice missing from VulkanDevice."); // Check internal consistency
        }

        /**
         * Internal static method called by recreate(). This performs the core
         * resource creation logic using the state from the existing swapchain instance.
         */
        private static void recreateInternal(VulkanSwapchain instance) {
            ...
            VulkanSwapchain recreated = createSwapchainResources(
                    instance, // Pass the existing instance to copy dependencies
        ...
        /**
         * Core logic to query support, choose formats/modes, and create all Vulkan resources.
         * Can be called from build() or recreateInternal().
         *
        ...
        private static VulkanSwapchain createSwapchainResources(Object source, boolean useVSync, long oldSwapchainHandle) {
            ...
            try (MemoryStack stack = stackPush()) {
                ...
                vkCheck(vkGetPhysicalDeviceSurfacePresentModesKHR(physDev, surf, presentModeCount, presentModes), "Failed query present modes");
                ...
                VkSwapchainCreateInfoKHR createInfo = VkSwapchainCreateInfoKHR.calloc(stack)
                        ...
                        .compositeAlpha(VK_COMPOSITE_ALPHA_OPAQUE_BIT_KHR) // Assume opaque is supported
                        .presentMode(chosenPresentMode)
                        .clipped(true)
                        .oldSwapchain(oldSwapchainHandle); // Pass old handle if provided
                ...
                Gdx.app.log("VulkanSwapchain.Builder", "Created swapchain resources. VSync: " + useVSync + ", Format: " + chosenFormat.format() + ", Extent: " + chosenExtent.width() + "x" + chosenExtent.height());

                // --- Construct VulkanSwapchain Instance ---
                // Use a dummy builder just to pass dependencies if called from recreate
                Builder dummyBuilder = (source instanceof Builder) ? (Builder) source : new Builder().device(dev).surface(surf).windowHandle(windowHnd).configuration(new VulkanApplicationConfiguration()); // Need config? Pass null?

                return new VulkanSwapchain(dummyBuilder, // Pass original builder or dummy
                        swapchainHandle, images, imageViews,
                        renderPassHandle, framebuffers,
                        chosenExtent, chosenFormat.format(), useVSync);
            }
        }
        ...
        private static VkExtent2D chooseSwapExtentInternal(VkSurfaceCapabilitiesKHR capabilities, long windowHandle) {
            try (MemoryStack stack = stackPush()) {
                if (capabilities.currentExtent().width() != 0xFFFFFFFF) {
                    ...
                    actualExtent.width(clamp(pWidth.get(0), capabilities.minImageExtent().width(), capabilities.maxImageExtent().width()));
                    actualExtent.height(clamp(pHeight.get(0), capabilities.minImageExtent().height(), capabilities.maxImageExtent().height()));
        ...
        private static List<Long> createFramebuffersInternal(VkDevice rawDevice, List<Long> imageViews, long renderPass, VkExtent2D extent) {
