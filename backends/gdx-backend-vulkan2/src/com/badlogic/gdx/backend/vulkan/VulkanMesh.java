...
public class VulkanMesh implements Disposable {
    ...
    public VulkanMesh(VulkanDevice device, long vmaAllocator, boolean isStatic, int maxVertices, int maxIndices, VertexAttributes attributes) {
    ...
    /**
     * Renders the mesh. Assumes pipeline and descriptors are already bound.
     * Call bind() first.
    ...
    public void render(VkCommandBuffer commandBuffer, int primitiveType, int offset, int count) {
        ...
        if (indices != null) {
            ...
            vkCmdDrawIndexed(commandBuffer, count, 1, offset, 0, 0); // (indexCount, instanceCount, firstIndex, vertexOffset, firstInstance)
        } else {
