...
/**
 * A {@link FillViewport} tailored for Vulkan.
 * <p>
 ...
 * which (for FillViewport, via ScalingViewport) calculates the correct screen bounds and then calls
 * {@code apply(boolean centerCamera)}. This call will resolve to the overridden
 * version in this class.
 */
public class VulkanFillViewport extends FillViewport {

    public VulkanFillViewport(float worldWidth, float worldHeight) {
        super(worldWidth, worldHeight);
    }

    public VulkanFillViewport(float worldWidth, float worldHeight, Camera camera) {
        super(worldWidth, worldHeight, camera);
    }

    /**
     * Applies the viewport to the camera and updates the camera.
     * This method is called by {@link com.badlogic.gdx.utils.viewport.ScalingViewport#update(int, int, boolean)}
     * (which FillViewport inherits) after screen bounds have been calculated.
     * <p>
    ...
    public void apply(boolean centerCamera) {
        ...
        // ScalingViewport.update() (via FillViewport.update()) before this method is called.
     ...
     * which is {@link com.badlogic.gdx.utils.viewport.ScalingViewport#update(int, int, boolean)}.
     * That method calculates the "fill" screen bounds and then calls
