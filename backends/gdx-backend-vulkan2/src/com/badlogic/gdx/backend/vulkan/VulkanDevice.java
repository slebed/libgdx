...
public class VulkanDevice implements VkResource {
	...
	private VulkanDevice (VkDeviceHandle deviceHandle, VkQueue graphicsQueue, VkPhysicalDevice physicalDevice,
		...
		/*
		 * try (MemoryStack stack = stackPush()) { VkCommandPoolCreateInfo poolInfo = VkCommandPoolCreateInfo.calloc(stack)
		 * .sType(VK_STRUCTURE_TYPE_COMMAND_POOL_CREATE_INFO) // Flag allows resetting individual command buffers
		 * .flags(VK_COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT) // Use the queue family index you used to create the device/queue
		 * .queueFamilyIndex(yourGraphicsQueueFamilyIndex); // Replace with actual index
		 * 
		 * LongBuffer pCommandPool = stack.mallocLong(1); VkMemoryUtil.vkCheck(vkCreateCommandPool(rawDeviceHandle, poolInfo, null,
		 * pCommandPool), "Failed to create command pool"); this.graphicsCommandPool = pCommandPool.get(0);
	...
	public static class Builder {
		...
		public VulkanDevice build () {
			try (MemoryStack stack = stackPush()) {
				...
				VkMemoryUtil.vkCheck(vkCreateDevice(physicalDevice, createInfo, null, pDevice), "Failed to create logical device");
				...
				VkMemoryUtil.vkCheck(vkCreateCommandPool(device, poolInfo, null, pCommandPool), "Failed to create command pool");
	...
	public long loadShaderModule (String filePath) {
		try (MemoryStack stack = MemoryStack.stackPush()) {
			...
			VkMemoryUtil.vkCheck(vkCreateShaderModule(this.deviceHandle, createInfo, null, pShaderModule),
	...
	public void endSingleTimeCommands (VkCommandBuffer commandBuffer) {
		try (MemoryStack stack = MemoryStack.stackPush()) {
			...
			VkMemoryUtil.vkCheck(vkQueueWaitIdle(graphicsQueue), "Queue wait idle failed after single time command");
