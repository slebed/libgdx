...
public class VulkanDeviceCapabilities {
    ...
    public VulkanDeviceCapabilities(VkPhysicalDevice physicalDevice) {
        ...
        try (MemoryStack stack = MemoryStack.stackPush()) {
            ...
            // This will include VkPhysicalDeviceMaintenance4Properties if API >= 1.3
            // and other version-specific property structs if needed in the future.
            ...
            if (this.apiVersion >= VK_API_VERSION_1_3) {
                ...
                // If other property structs were needed, they would be chained here:
                // maintenance4Properties.pNext(nextPropertyStruct.address());
            }
            ...
            // else properties2.pNext is NULL, which is fine.

            // Call vkGetPhysicalDeviceProperties2 (or KHR equivalent) to populate properties2
            // and any chained property structs (like maintenance4Properties).
            VkInstance instance = physicalDevice.getInstance();
            VKCapabilitiesInstance capabilitiesInstance = instance.getCapabilities();
            if (capabilitiesInstance.vkGetPhysicalDeviceProperties2 != NULL) {
                if (DEBUG_LOGGING && Gdx.app != null) Gdx.app.log(TAG, "Using core vkGetPhysicalDeviceProperties2 for extended properties.");
                VK11.vkGetPhysicalDeviceProperties2(physicalDevice, properties2);
            } else if (capabilitiesInstance.vkGetPhysicalDeviceProperties2KHR != NULL) {
                ...
                // populated by vkGetPhysicalDeviceProperties via the initial call if extended calls fail.
                // The most robust way is to ensure vkGetPhysicalDeviceProperties2 or its KHR is available
                // by enabling VK_KHR_get_physical_device_properties2 instance extension.
            }

            // Store persistent copy of core properties (from properties2.properties() which was just filled)
            ...
            // Initialize limits from the persistent copy
            ...
            if (maintenance4Properties != null && properties2.pNext() == maintenance4Properties.address()) {
                // This check confirms maintenance4Properties was indeed part of the chain that got populated.
                this.maxBufferSize = maintenance4Properties.maxBufferSize();
