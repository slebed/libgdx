...
public class VulkanTexture extends Texture {
    ...
    public VulkanTexture(FileHandle file) {
        ...
        try {
            ...
            stagingBuffer = VulkanResourceUtil.createManagedBuffer(vmaAllocator, imageSize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VMA_MEMORY_USAGE_AUTO, VMA_ALLOCATION_CREATE_HOST_ACCESS_SEQUENTIAL_WRITE_BIT | VMA_ALLOCATION_CREATE_MAPPED_BIT);
            ...
            tempGpuImage = VulkanResourceUtil.createManagedImage(vmaAllocator, texWidth, texHeight, vkFormat, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT, VMA_MEMORY_USAGE_AUTO_PREFER_DEVICE, 0);
    ...
    /**
     * Creates a VulkanTexture directly from a Pixmap.
     * Assumes Vulkan backend is initialized and active.
     * The provided Pixmap is NOT disposed by this constructor.
     * If the Pixmap is not RGBA8888, a temporary copy will be created and disposed internally.
     *
    ...
    public VulkanTexture(Pixmap pixmap) {
        ...
        Gdx.app.log(logTag, "(Constructor) Creating texture from Pixmap (" + pixmap.getWidth() + "x" + pixmap.getHeight() + ")");
        ...
        try {
            ...
            if (pixmap.getFormat() != Pixmap.Format.RGBA8888) {
                Gdx.app.log(logTag, "Converting input Pixmap to RGBA8888 for upload...");
            ...
            Gdx.app.log(logTag, "VMA Image created and data uploaded from Pixmap.");
            ...
            Gdx.app.log(logTag, "VulkanTexture created successfully from Pixmap.");

        } catch (Exception e) {
            // Cleanup intermediate resources if constructor failed
            Gdx.app.error(logTag, "Failed during VulkanTexture constructor from Pixmap", e);
            // Destroy in reverse order of creation attempt
            if (tempSampler != VK_NULL_HANDLE && retrievedDevice != null) vkDestroySampler(retrievedDevice.getRawDevice(), tempSampler, null);
            if (tempImageView != VK_NULL_HANDLE && retrievedDevice != null) vkDestroyImageView(retrievedDevice.getRawDevice(), tempImageView, null);
            if (tempGpuImage != null) tempGpuImage.dispose(); // Calls vmaDestroyImage
