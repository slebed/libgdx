...
public class VulkanSpriteBatch implements Batch, Disposable {
	...
	public VulkanSpriteBatch (int size) {
		...
		for (int i = 0; i < descriptorSetCount; i++) {
			...
			VulkanDescriptorManager.updateUniformBuffer(rawDevice, // Pass VkDevice
	...
	private void updateTextureDescriptor (long descriptorSet, VulkanTexture texture) {
		...
		// Gdx.app.log(TAG, "Updating Texture descriptor for set " + descriptorSet + " with TexView: " +
		// texture.getImageViewHandle() + " and Sampler: " + texture.getSamplerHandle());
		try (MemoryStack stack = MemoryStack.stackPush()) {
	...
	private void drawInternal (VulkanTexture texture, float[] spriteVertices, int numFloats) {
		...
		try {
			...
			Gdx.app.error(TAG, "BufferOverflowException during vertices.put. vertexBufferIdx=" + vertexBufferIdx + ", numFloats="
				+ numFloats + ", capacity=" + vertices.capacity(), e);
			...
			Gdx.app.error(TAG, "IllegalArgumentException during vertices.put. vertexBufferIdx=" + vertexBufferIdx + ", numFloats="
				+ numFloats + ", capacity=" + vertices.capacity(), e);
	...
	/*
	 * private long getCurrentRenderPassHandle() { if (Gdx.graphics instanceof VulkanGraphics) { VulkanGraphics gfx =
	 * (VulkanGraphics) Gdx.graphics; // Ensure VulkanGraphics has a getter for the swapchain VulkanSwapchain swapchain =
	 * gfx.getSwapchain(); if (swapchain != null) { // Ensure VulkanSwapchain has a getter for the render pass long rpHandle =
	 * swapchain.getRenderPass(); if (rpHandle != VK_NULL_HANDLE) { return rpHandle; } else { throw new
	 * GdxRuntimeException("VulkanSpriteBatch: Failed to get RenderPass handle from VulkanSwapchain (handle is VK_NULL_HANDLE)"); }
	 * } else { throw new GdxRuntimeException("VulkanSpriteBatch: Failed to get VulkanSwapchain from VulkanGraphics"); } } else {
	...
	public void flush () {
		...
		// Gdx.app.log(TAG, "Flush Check: RP Handle obtained = " + currentRenderPassHandle + ", VK_NULL_HANDLE = " +
		// VK_NULL_HANDLE);
		...
		// }
		if (currentRenderPassHandle == VK_NULL_HANDLE) {
			Gdx.app.error(TAG, "Flush Error: Condition (currentRenderPassHandle == VK_NULL_HANDLE) was TRUE! Handle value was: "
				+ currentRenderPassHandle); // Log if condition is true
			throw new GdxRuntimeException("Cannot flush() batch, current render pass handle is null.");
		} else {
			// Gdx.app.log(TAG, "Flush Check: Condition (currentRenderPassHandle == VK_NULL_HANDLE) was FALSE. Handle is valid."); //
			// Log if condition is false
		}
