...
public class VulkanStage extends Stage {
	 ...
	 * <p>
	 * Assumes that {@code batch.begin(VkCommandBuffer)} has already been called externally and that a Vulkan render pass is active
	 * on that command buffer. This method does NOT call {@code batch.begin()} or {@code batch.end()}. */
	@Override
	public void draw () {
		...
		/*
		 * float w1 = Gdx.graphics.getWidth(); float h1 = Gdx.graphics.getHeight();
		 * 
		 * if (w1 <= 0 || h1 <= 0) { // Use <= 0 for safety Gdx.app.error("VulkanStage",
		 * "WIDTH OR HEIGHT IS ZERO or invalid! Skipping matrix update."); System.exit(0); }
		 * 
		 * if (!getRoot().isVisible()) return;
		 * 
		 * Batch batch = getBatch(); // Get the batch instance stored in the superclass if (batch != null) { float w =
		 * Gdx.graphics.getWidth(); float h = Gdx.graphics.getHeight(); float near = 0.0f; float far = 1.0f;
		 * 
		 * // The manual calculation block setting vulkanOrthoMatrix.val[...] should be here... // Make sure these lines result in
		 * M22 = 1.0f / (far - near) and M32 = -near / (far - near) // For near=0, far=1, this means M22 = 1.0, M32 = 0.0
		 * 
		 * vulkanOrthoMatrix.val[Matrix4.M00] = 2.0f / w; vulkanOrthoMatrix.val[Matrix4.M11] = 2.0f / h;
		 * vulkanOrthoMatrix.val[Matrix4.M22] = 1.0f / (far - near); // Vulkan Z maps [0,1] input -> [0,1] NDC ( = 1.0f)
		 * vulkanOrthoMatrix.val[Matrix4.M30] = -1.0f; // Translate X vulkanOrthoMatrix.val[Matrix4.M31] = -1.0f; // Translate Y
		 * vulkanOrthoMatrix.val[Matrix4.M32] = -near / (far - near); // Vulkan Z ( = 0.0f) vulkanOrthoMatrix.val[Matrix4.M33] =
		 * 1.0f; // Clear others... vulkanOrthoMatrix.val[Matrix4.M01] = 0f; vulkanOrthoMatrix.val[Matrix4.M02] = 0f;
		 * vulkanOrthoMatrix.val[Matrix4.M03] = 0f; vulkanOrthoMatrix.val[Matrix4.M10] = 0f; vulkanOrthoMatrix.val[Matrix4.M12] =
		 * 0f; vulkanOrthoMatrix.val[Matrix4.M13] = 0f; vulkanOrthoMatrix.val[Matrix4.M20] = 0f; vulkanOrthoMatrix.val[Matrix4.M21]
		 * = 0f; vulkanOrthoMatrix.val[Matrix4.M23] = 0f;
		 * 
		 * batch.setProjectionMatrix(vulkanOrthoMatrix);
		 * 
		 * if (!getRoot().isVisible()) return;
		 * 
		 * if (batch.isDrawing()) { getRoot().draw(batch, 1.0f); } else { Gdx.app.error("VulkanStage",
		 * "Stage.draw() called but Batch is not drawing!"); } } else { Gdx.app.error("VulkanStage",
