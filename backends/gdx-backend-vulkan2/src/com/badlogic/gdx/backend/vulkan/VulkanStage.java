...
public class VulkanStage extends Stage {
    ...
    /**
     * Creates a VulkanStage with a {@link VulkanScreenViewport}, creating and owning its own
     * internal {@link VulkanSpriteBatch}. This Stage will dispose the internally
     * created batch when the stage itself is disposed.
     * <p>
     * Note: Requires VulkanScreenViewport or similar to be available.
     * If VulkanScreenViewport isn't defined, use `new VulkanStage(new ScreenViewport())`
     * and ensure ScreenViewport works correctly with your Vulkan backend's camera setup.
    ...
    /**
     * Creates a VulkanStage with the specified viewport, creating and owning its own
     * internal {@link VulkanSpriteBatch}. This Stage will dispose the internally
     * created batch when the stage itself is disposed.
     *
    ...
    /**
     * Creates a VulkanStage with the specified viewport and an externally provided batch.
     * This Stage will NOT own or dispose the provided batch.
     *
     * @param viewport The viewport to manage the stage's camera and projection.
    ...
    private void drawActorDebugVulkan(Actor actor, Batch batch) {
        ...
        if (actor.getOriginX() != 0 || actor.getOriginY() != 0 || actor.getRotation() != 0 || actor.getScaleX() != 1 || actor.getScaleY() != 1) {
    ...
    public boolean touchDown(int screenX, int screenY, int pointer, int button) {
        ...
        if (viewport != null) {
            System.out.println("    Viewport State: screenX=" + viewport.getScreenX() + ", screenY=" + viewport.getScreenY() +
                    ", screenWidth=" + viewport.getScreenWidth() + ", screenHeight=" + viewport.getScreenHeight() +
                    ", worldWidth=" + viewport.getWorldWidth() + ", worldHeight=" + viewport.getWorldHeight());
            ...
            if (camera != null) {
                System.out.println("    Camera State (pos/dir/up): pos=" + camera.position + ", dir=" + camera.direction + ", up=" + camera.up);
            }
        }
...
public class VulkanStage extends Stage {
    ...
    private final Vector2 tmpCoords = new Vector2();
    private final Color tmpColor = new Color();
    private Vector2 tempScreenCoords=new Vector2();
    private Vector2 tempStageCoords= new Vector2();
...
/**
    ...
    public void draw() {
        ...
        Batch batch = getBatch(); // Should be VulkanSpriteBatch
        ...
        // 2. Begin batch (VulkanStage class comment implies begin/end are external,
        //    but the original draw() method called them. Let's keep that pattern.)
        boolean batchWasDrawing = batch.isDrawing(); // Should ideally be false here
        if (!batchWasDrawing) {
            batch.begin();
        } else {
            // If it was already drawing, maybe flush? Or log warning?
            // Gdx.app.log(TAG, "Warning: Batch was already drawing in Stage.draw()");
        }
        ...
        // 4. Draw debug visuals if enabled, BEFORE ending the batch
        if (isDebugAll() && whitePixel != null) { // Use the stage's debug flag
            try {
                drawDebugVulkanRecursive(getRoot(), batch); // Call the recursive debug draw
            } catch (Exception e) {
                Gdx.app.error(TAG, "Error during debug draw: " + e.getMessage(), e);
                // Potentially end batch here if error is severe
            }
        }

        // 5. End batch if we started it (or if it was always meant to be ended here)
        if (!batchWasDrawing) {
            batch.end();
        } else {
            // If it was already drawing, should we end it? Based on class comment, maybe not.
            // This depends heavily on how the external render loop uses the stage.
            // For consistency with original code block, we will end it.
            batch.end();
        }

        VulkanApplication app = (VulkanApplication) Gdx.app;
        if (app.getCurrentWindow().runOnce) {
            //Gdx.app.log(TAG, app.getCurrentWindow().getConfig().title + "\tCamera: " + viewport.getScreenWidth() + " x " + viewport.getScreenHeight());
    ...
    public void dispose() {
        ...
        if (managesInternalBatch && batch != null) {
            Gdx.app.log(TAG, "Disposing internally managed VulkanSpriteBatch.");
            batch.dispose();
            // Important: Set batch to null in superclass AFTER disposing it
            // super.setBatch(null); // Requires Stage.setBatch to be accessible or use reflection
        } else {
            Gdx.app.log(TAG, "Dispose: Not disposing batch (managed externally or null). managesInternalBatch=" + managesInternalBatch);
        }
...
/**
    ...
    private void drawDebugVulkanRecursive(Actor actor, Batch batch) {
        ...
        // If the actor is invisible, only draw debug if its own debug flag is true.
        // (Standard behavior often skips invisible actors entirely when isDebugAll is true,
        // but respecting the actor's specific flag seems reasonable).
        if (!actorIsVisible && !actor.getDebug()) {
            return;
        }
...
/**
    ...
    private void drawActorDebugVulkan(Actor actor, Batch batch) {
        ...
        // Use a fixed debug color since actor.getDebugColor() doesn't exist.
        // Let's use a semi-transparent blue for bounds.
        float debugAlpha = 0.4f;
        batch.setColor(0, 0, 1, debugAlpha); // Blue, 40% opacity
        ...
        // Draw filled rectangle representing logical bounds (ignores actor transform)
        if (whitePixel != null) {
            // Draw the rectangle covering the actor's logical area at its stage position
            batch.draw(whitePixel, stageX, stageY, width, height);
        } else {
            Gdx.app.error(TAG, "whitePixel texture is null during debug draw!");
            batch.setColor(tmpColor);
        ...
        //if (actor.getOriginX() != 0 || actor.getOriginY() != 0 || actor.getRotation() != 0 || actor.getScaleX() != 1 || actor.getScaleY() != 1) {
        // Calculate origin position in STAGE coordinates
        tmpCoords.set(actor.getOriginX(), actor.getOriginY());
        actor.localToStageCoordinates(tmpCoords);

        // Draw marker slightly more opaque and yellow
        batch.setColor(Color.YELLOW.r, Color.YELLOW.g, Color.YELLOW.b, Math.max(debugAlpha, 0.8f));
        // Draw 5x5 square centered roughly on origin
        batch.draw(whitePixel, tmpCoords.x - 2.5f, tmpCoords.y - 2.5f, 5, 5);
        //}
...
/**
    ...
    public boolean touchDown(int screenX, int screenY, int pointer, int button) {
        // --- START DEBUG LOGGING ---
        System.out.println("_____________________\n>>> VulkanStage.touchDown Received: screen(" + screenX + ", " + screenY + ")"); // Raw coords received

        Viewport viewport = getViewport();
        Camera camera = getCamera();

        if (viewport != null) {
            // Log the viewport state *before* unprojection
            System.out.println("    Viewport State: screenX=" + viewport.getScreenX() + ", screenY=" + viewport.getScreenY() +
                    ", screenWidth=" + viewport.getScreenWidth() + ", screenHeight=" + viewport.getScreenHeight() +
                    ", worldWidth=" + viewport.getWorldWidth() + ", worldHeight=" + viewport.getWorldHeight());

            // Perform the conversion
            tempScreenCoords.set(screenX, screenY);
            // Use the viewport's unproject method directly for clarity, or stage.screenToStageCoordinates
            viewport.unproject(tempScreenCoords); // Modifies tempScreenCoords in-place to stage coords
            tempStageCoords.set(tempScreenCoords); // Copy result to avoid modification if screenToStageCoords is used later

            System.out.println("    Calculated Stage Coords: (" + tempStageCoords.x + ", " + tempStageCoords.y + ")");

            // Log camera state if relevant
            if (camera != null) {
                System.out.println("    Camera State: pos=" + camera.position + ", dir=" + camera.direction + ", up=" + camera.up);
                // Logging camera.combined matrix might be too verbose unless needed
            }

        } else {
            System.out.println("    Viewport is NULL, cannot calculate stage coordinates or log state.");
        }
        // --- END DEBUG LOGGING ---
    ...
    @Override
    public boolean mouseMoved(int screenX, int screenY) {
        //System.out.println(">>> Window 2 InputProcessor: Raw Event screen Coords: (" + screenX + ", " + screenY + ")");
        return super.mouseMoved(screenX, screenY); // Or whatever your handler does
    }
}*/
