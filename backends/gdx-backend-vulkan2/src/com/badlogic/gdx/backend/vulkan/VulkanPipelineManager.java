...
public class VulkanPipelineManager implements Disposable {
	...
	/** Creates the default graphics pipeline and its layout based on shader files. Cleans up any existing default pipeline/layout
	 * first. Loads shader modules from the provided FileHandles, utilizing an internal cache. Expects descriptor set layout to be
	 * created beforehand.
	 *
	...
	public void createDefaultPipeline (FileHandle vertShaderFile, FileHandle fragShaderFile, long descriptorSetLayoutHandle,
		...
		try (MemoryStack stack = stackPush()) {
			...
			VkGraphicsPipelineCreateInfo.Buffer pipelineInfo = VkGraphicsPipelineCreateInfo.calloc(1, stack).sType$Default()
				...
				.subpass(0).basePipelineHandle(VK_NULL_HANDLE).basePipelineIndex(-1);
