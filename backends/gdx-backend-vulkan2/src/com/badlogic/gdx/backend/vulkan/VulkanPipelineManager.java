...
public class VulkanPipelineManager implements Disposable {
	...
	/** Creates the default graphics pipeline based on shader files and layouts. Cleans up any existing default graphics pipeline
	 * first. The pipeline layout lifecycle is managed by the cache via getOrCreatePipelineLayout. Loads shader modules from the
	 * provided FileHandles, utilizing an internal cache. Expects descriptor set layout to be created beforehand.
	 *
	...
	public void createDefaultPipeline (FileHandle vertShaderFile, FileHandle fragShaderFile, long descriptorSetLayoutHandle,
		...
		try (MemoryStack stack = stackPush()) {
			...
			VkPipelineVertexInputStateCreateInfo vertexInputInfo = createVertexInputInfo(stack); // Ensure this matches SpriteBatch
																																// vertex format if used here
			...
			VkGraphicsPipelineCreateInfo.Buffer pipelineInfo = VkGraphicsPipelineCreateInfo.calloc(1, stack).sType$Default()
				...
				.pColorBlendState(colorBlending).pDynamicState(dynamicState) // null if no dynamic states
	...
	private VkPipelineShaderStageCreateInfo.Buffer createShaderStages (MemoryStack stack, long vertModuleHandle,
	...
	private VkPipelineRasterizationStateCreateInfo createRasterizationInfo (MemoryStack stack) {
		return VkPipelineRasterizationStateCreateInfo.calloc(stack).sType$Default().depthClampEnable(false)
			.rasterizerDiscardEnable(false).polygonMode(VK_POLYGON_MODE_FILL).lineWidth(1.0f).cullMode(VK_CULL_MODE_NONE) // No
																																								// culling
																																								// for
																																								// simple
																																								// quad
	...
	private VkPipelineColorBlendStateCreateInfo createColorBlendInfo (MemoryStack stack) {
		...
		return VkPipelineColorBlendStateCreateInfo.calloc(stack).sType$Default().logicOpEnable(false)
	...
	/** Gets or creates a graphics pipeline specifically configured for VulkanSpriteBatch. Assumes standard alpha blending and
	 * vertex layout defined in VulkanSpriteBatch. NOTE: Simple implementation assumes pipeline compatibility with the provided
	 * renderPassHandle. More complex caching might be needed if used with multiple different render passes.
	 *
	...
	public synchronized long getOrCreateSpriteBatchPipeline (long batchDescriptorSetLayoutHandle, long renderPassHandle) {
		...
		// Assumes renderPassHandle is compatible. If not, caching needs to be more complex (e.g., Map<Long, Long>
		// renderPassToPipelineCache)
		...
		try (MemoryStack stack = stackPush()) {
			...
			VkPipelineShaderStageCreateInfo.Buffer shaderStages = createShaderStages(stack, vertModuleHandle, fragModuleHandle); // Standard
																																										// helper
																																										// likely
																																										// OK
			...
			attributeDescriptions.get(1) // Color (packed float -> interpreted as vec4 in shader)
				.binding(0).location(1).format(VK_FORMAT_R32_SFLOAT).offset(VulkanSpriteBatch.POSITION_COMPONENTS * Float.BYTES); // Offset
																																										// 8
			attributeDescriptions.get(2) // TexCoord (vec2)
				.binding(0).location(2).format(VK_FORMAT_R32G32_SFLOAT)
				.offset((VulkanSpriteBatch.POSITION_COMPONENTS + VulkanSpriteBatch.COLOR_COMPONENTS) * Float.BYTES); // Offset 12 (2*4
																																						// + 1*4)
			...
			colorBlendAttachment.get(0)
				...
				.colorBlendOp(VK_BLEND_OP_ADD).srcAlphaBlendFactor(VK_BLEND_FACTOR_ONE) // Optional: Often One/Zero or
																												// One/OneMinusSrcAlpha for alpha
			...
			VkPipelineColorBlendStateCreateInfo colorBlending = VkPipelineColorBlendStateCreateInfo.calloc(stack).sType$Default()
				.logicOpEnable(false).attachmentCount(colorBlendAttachment.remaining()).pAttachments(colorBlendAttachment);
			...
			VkGraphicsPipelineCreateInfo.Buffer pipelineInfo = VkGraphicsPipelineCreateInfo.calloc(1, stack).sType$Default()
				...
				.layout(pipelineLayoutHandle).renderPass(renderPassHandle).subpass(0).basePipelineHandle(VK_NULL_HANDLE)
				.basePipelineIndex(-1);
