...
public class VulkanApplication implements VulkanApplicationBase {
	...
	public VulkanApplication (ApplicationListener listener, VulkanApplicationConfiguration config) {
		...
		VulkanGraphics graphics = new VulkanGraphics(windowHandle, config, this.vulkanDevice, // Pass the VulkanDevice object
		...
		try {
			...
			if (initialWidth > 0 && initialHeight > 0) {
				Gdx.app.log("VulkanApplication",
					"Calling listener.resize() with initial dimensions: " + initialWidth + "x" + initialHeight);
				...
				Gdx.app.error("VulkanApplication", "Initial dimensions from Gdx.graphics are invalid (" + initialWidth + "x"
					+ initialHeight + "), skipping initial resize call!");
			}
		} catch (Throwable e) {
