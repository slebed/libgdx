...
public class VulkanApplication implements VulkanApplicationBase {
	...
	private void initializeVulkanCore (long windowHandle) {
		try (MemoryStack stack = MemoryStack.stackPush()) {
			...
			this.vulkanInstance = new VulkanInstance.Builder().setApplicationName(config.title)
			...
			this.vkDevice = new VkDevice.Builder().setPhysicalDevice(physicalDevice).setQueueFamilyIndex(indices.graphicsFamily) // Assuming
																																										// graphics==present
																																										// for
																																										// now
	...
	public VulkanApplication (ApplicationListener listener, VulkanApplicationConfiguration config) {
		...
		VulkanWindow window = new VulkanWindow(listener, lifecycleListeners, config, this);// , graphics, input); // Window takes
																														// graphics/input
	...
	protected void cleanup () {
		...
		if (debugMessenger != VK_NULL_HANDLE && vulkanInstance != null) {
			...
			if (Gdx.app != null)
				Gdx.app.log("VulkanApplication", "Debug messenger destroyed.");
			else
				System.out.println("VulkanApplication: Debug messenger destroyed.");
		...
		if (debugCallbackInstance != null) {
			...
			if (Gdx.app != null)
				Gdx.app.log("VulkanApplication", "Debug callback freed.");
			else
				System.out.println("VulkanApplication: Debug callback freed.");
		}
	...
	private VulkanWindow createWindow (final VulkanApplicationConfiguration config, ApplicationListener listener,
	 ...
	 * VkDebugUtilsMessengerCallbackDataEXT callbackData = VkDebugUtilsMessengerCallbackDataEXT.create(pCallbackData); String
	 * severity; if ((messageSeverity & VK_DEBUG_UTILS_MESSAGE_SEVERITY_ERROR_BIT_EXT) != 0) { severity = "ERROR"; } else if
	 * ((messageSeverity & VK_DEBUG_UTILS_MESSAGE_SEVERITY_WARNING_BIT_EXT) != 0) { severity = "WARNING"; } else if
	 * ((messageSeverity & VK_DEBUG_UTILS_MESSAGE_SEVERITY_INFO_BIT_EXT) != 0) { // Add INFO level if desired severity = "INFO"; }
	 * else if ((messageSeverity & VK_DEBUG_UTILS_MESSAGE_SEVERITY_VERBOSE_BIT_EXT) != 0) { severity = "VERBOSE"; } else { severity
	 * = "UNKNOWN"; } System.err.println("VULKAN " + severity + ": " + callbackData.pMessageString()); // You could add breakpoints
	 * here based on severity for debugging return VK_FALSE; // Must return VK_FALSE } };
	 */
