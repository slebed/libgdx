...
public class VulkanApplication implements VulkanApplicationBase {
	...
	public VulkanApplication (ApplicationListener listener, VulkanApplicationConfiguration appConfig) {
		...
		// if (this.vulkanDevice == null || this.vulkanDevice.getRawDevice() == null || this.vulkanDevice.getRawDevice().address()
		// == VK_NULL_HANDLE) {
		...
		VulkanGraphics primaryGraphics = new VulkanGraphics(windowHandle, this.appConfig, // Use the app config
			this, // Pass application reference
			this.vulkanDevice, this.vmaAllocator, this.pipelineManager, // Pass shared manager
		...
		VulkanWindow window = new VulkanWindow(listener, lifecycleListeners, this.appConfig, // Or a window-specific config based on
																															// appConfig?
			this, this.primarySurface, // Surface from initializeVulkanCore
		...
		/*
		 * try { Gdx.app.log(TAG, "Calling primary listener.create()..."); listener.create(); Gdx.app.log(TAG,
		 * "Primary listener.create() completed.");
		 * 
		 * final int initialWidth = Gdx.graphics.getWidth(); final int initialHeight = Gdx.graphics.getHeight(); if (initialWidth >
		 * 0 && initialHeight > 0) { Gdx.app.log(TAG, "Calling primary listener.resize() with initial dimensions: " + initialWidth +
		 * "x" + initialHeight); listener.resize(initialWidth, initialHeight); Gdx.app.log(TAG,
		 * "Initial primary listener.resize() completed."); } else { Gdx.app.error(TAG,
		 * "Initial dimensions from Gdx.graphics are invalid (" + initialWidth + "x" + initialHeight +
		 * "), skipping initial resize call!"); } } catch (Throwable e) { Gdx.app.error(TAG,
		 * "Exception occurred during initial ApplicationListener.resize()", e); cleanup(); throw new
		 * GdxRuntimeException("Exception occurred during initial ApplicationListener.resize()", e); }
		 */

		/*
		 * try { Gdx.app.log(TAG, "Calling listener.create()..."); listener.create(); // Calls VulkanTestChooser.create() ->
		 * setInputProcessor() Gdx.app.log(TAG, "listener.create() completed."); } catch (Throwable e) { cleanup(); throw new
		 * GdxRuntimeException("Exception occurred in ApplicationListener.create()", e); }
		 * 
		 * try { final int initialWidth = Gdx.graphics.getWidth(); final int initialHeight = Gdx.graphics.getHeight(); if
		 * (initialWidth > 0 && initialHeight > 0) { Gdx.app.log(TAG, "Calling listener.resize() with initial dimensions: " +
		 * initialWidth + "x" + initialHeight); listener.resize(initialWidth, initialHeight); Gdx.app.log(TAG,
		 * "Initial listener.resize() completed."); } else { Gdx.app.error(TAG, "Initial dimensions from Gdx.graphics are invalid ("
		 * + initialWidth + "x" + initialHeight + "), skipping initial resize call!"); } } catch (Throwable e) { Gdx.app.error(TAG,
		 * "Exception occurred during initial ApplicationListener.resize()", e); cleanup(); throw new
		 * GdxRuntimeException("Exception occurred during initial ApplicationListener.resize()", e); }
		 */
		Gdx.app.log(TAG,
	...
	/*
	 * public void loop() { Array<VulkanWindow> closedWindows = new Array<>(); long frameCount = 0;
	 * 
	 * if (lastFrameTime == -1) { // Check if first frame lastFrameTime = System.nanoTime(); } if (frameCounterStart == 0) { //
	 * Check if FPS counter needs init frameCounterStart = System.nanoTime(); }
	 * 
	 * if (windows.size > 0) { currentWindow = windows.first(); if (currentWindow != null) { // Ensure context is active for
	 * initial operations if needed currentWindow.makeCurrent(); } else { Gdx.app.error(TAG,
	 * "Window array has size > 0 but first element is null!"); // Handle this error state appropriately } }
	 * 
	 * // Main loop continues as long as the app is running and has windows while (running && windows.size > 0) {
	 * 
	 * long time = System.nanoTime();
	 * 
	 * if (resetDeltaTime) { resetDeltaTime = false; deltaTime = 0; } else { // Calculate delta time in seconds deltaTime = (time -
	 * lastFrameTime) / 1000000000.0f; } lastFrameTime = time;
	 * 
	 * // FPS calculation if (time - frameCounterStart >= 1000000000) { fps = frames; frames = 0; frameCounterStart = time; }
	 * frames++; frameId++;
	 * 
	 * if (audio != null) { audio.update(); }
	 * 
	 * if (++frameCount % 120 == 0) { //Gdx.app.log(TAG,"loop() iteration: " + frameCount); }
	 * 
	 * GLFW.glfwPollEvents();
	 * 
	 * VulkanInput currentInput = null; if (currentWindow != null) { currentInput = currentWindow.getInput(); currentWindow.runOnce
	 * = true; }
	 * 
	 * if (currentInput != null) { InputProcessor loopProc = currentInput.getInputProcessor(); currentInput.update();
	 * currentInput.prepareNext(); } else { // Log if input couldn't be processed (e.g., no current window)
	 * //Gdx.app.log("VulkanAppLoop", "currentInput is NULL (currentWindow=" + currentWindow + "), skipping input processing."); }
	 * 
	 * boolean shouldRequestRendering; synchronized (runnables) { shouldRequestRendering = runnables.size > 0; if
	 * (shouldRequestRendering) { executedRunnables.clear(); executedRunnables.addAll(runnables); runnables.clear(); } } if
	 * (shouldRequestRendering) { for (Runnable runnable : executedRunnables) { try { runnable.run(); } catch (Throwable t) {
	 * Gdx.app.error(TAG, "Exception occurred in runnable execution", t); } }
	 * 
	 * for (VulkanWindow window : windows) { if (!window.getGraphics().isContinuousRendering()) window.requestRendering(); } }
	 * 
	 * boolean haveWindowsRendered = false; closedWindows.clear(); int targetFramerate = -2; // Use -2 to indicate not yet set this
	 * frame
	 * 
	 * for (VulkanWindow window : windows) { if (currentWindow != window) { window.makeCurrent(); currentWindow = window; }
	 * 
	 * if (targetFramerate == -2) targetFramerate = appConfig.foregroundFPS;//window.getConfig().foregroundFPS;
	 * 
	 * synchronized (lifecycleListeners) { try { haveWindowsRendered |= window.update(); } catch (Throwable t) { Gdx.app.error(TAG,
	 * "Exception occurred during window update/render", t); } }
	 * 
	 * if (window.shouldClose()) { closedWindows.add(window); } }
	 * 
	 * if (closedWindows.size > 0) { for (VulkanWindow closedWindow : closedWindows) { if (windows.size == 1) { for (int i =
	 * lifecycleListeners.size - 1; i >= 0; i--) { LifecycleListener l = lifecycleListeners.get(i); try { l.pause(); l.dispose(); }
	 * catch (Throwable t) { Gdx.app.error(TAG, "Exception occurred during lifecycle listener pause/dispose", t); } }
	 * lifecycleListeners.clear(); }
	 * 
	 * try { closedWindow.dispose(); } catch (Throwable t) { Gdx.app.error(TAG, "Exception occurred during window dispose", t); }
	 * 
	 * windows.removeValue(closedWindow, true);
	 * 
	 * if (currentWindow == closedWindow) { currentWindow = null; } }
	 * 
	 * if (currentWindow == null && windows.size > 0) { currentWindow = windows.first(); if (currentWindow != null) {
	 * currentWindow.makeCurrent(); } else { Gdx.app.error(TAG,
	 * "Window array has size > 0 but first element is null after closing window!"); // This indicates a potential issue with
	 * window removal or list management } } }
	 * 
	 * if (!haveWindowsRendered) { try { Thread.sleep(1000 / appConfig.idleFPS); } catch (InterruptedException e) { // ignore } }
	 * else if (targetFramerate > 0) { // Optional: Implement frame limiting if needed // sync.sync(targetFramerate); // Example
	 * using hypothetical Sync class } } // End while loop
	 * 
	 * Gdx.app.log(TAG, "loop() finished after " + frameCount + " iterations."); }
	 */

	/** Main application loop using direct, synchronized iteration as a workaround for the SnapshotArray issue. */
	public void loop () {
		...
		while (running && windows.size > 0) { // Loop while running and there are windows left
			...
			synchronized (windows) { // Synchronize on the actual list being iterated
				...
				for (int i = 0; i < currentSize; i++) { // Iterate up to the size captured
					...
					Gdx.app.log(TAG, "LOOP: DIRECT Processing window iteration i=" + i + ", handle=" + currentHandle + ", hash="
						+ window.hashCode());
					...
					if (windowGraphics == null || windowInput == null) {
						Gdx.app.error(TAG, "Skipping update for window " + currentHandle + " due to missing Graphics (" + windowGraphics
							+ ") or Input (" + windowInput + ") instance.");
					...
					Gdx.app.log(TAG, "LOOP: DIRECT Window " + currentHandle + " check: continuous=" + continuous + ", requested="
						+ requested + ", needsRender=" + needsRender + ", isIconified=" + iconified);
					// --- End Check Rendering ---
					...
					if (needsRender && !iconified) {
						...
						Gdx.app.log(TAG, "LOOP: DIRECT --- Skipping update() for window " + currentHandle + " --- (Reason: needsRender="
							+ needsRender + ", isIconified=" + iconified + ")");
					}
			...
			if (!closedWindows.isEmpty()) { // Use isEmpty()
				...
				synchronized (windows) { // Synchronize modifications to the main list and map
					...
					for (VulkanWindow closedWindow : closedWindows) {
						...
						try {
							...
							if (!removed) {
								Gdx.app.error(TAG, "Attempted to remove window " + closedHandle
									+ " but it wasn't found in 'windows' list during closure.");
							}
				...
				if (!currentWindowStillValid) {
					...
					if (this.currentWindow != null) {
						...
						Gdx.app.log(TAG,
							"Processing closed windows: Reset current window to handle " + this.currentWindow.getWindowHandle()
								+ " Gdx.graphics=" + (Gdx.graphics == null ? "null" : Gdx.graphics.hashCode()) + " Gdx.input="
								+ (Gdx.input == null ? "null" : Gdx.input.hashCode()));
					} else {
						// No windows left
						Gdx.graphics = null;
						Gdx.input = null;
	...
	protected void cleanup () {
		...
		/*
		 * if (Gdx.graphics instanceof VulkanGraphics) { System.out.println("[" + TAG + "] Disposing VulkanGraphics..."); try {
		 * ((VulkanGraphics) Gdx.graphics).dispose(); Gdx.app.log(TAG, "VulkanGraphics disposed."); } catch (Throwable t) {
		 * Gdx.app.log(TAG, "ERROR: Exception during graphics dispose."); t.printStackTrace(); } } else if (Gdx.graphics != null) {
		 ...
		 * KHRSurface.vkDestroySurfaceKHR(vulkanInstance.getRawInstance(), primarySurface, null); System.out.println("[" + TAG +
		 * "] Primary surface destroyed."); primarySurface = VK_NULL_HANDLE; } else if (primarySurface != VK_NULL_HANDLE) {
	...
	public VulkanWindow newWindow (ApplicationListener listener, VulkanWindowConfiguration config) {
		...
		VulkanGraphics newGraphicsInstance = new VulkanGraphics(newWindowHandle, // Use the NEW handle
...
// Also verify it's added to windowGraphicsMap, etc.
		...
		Gdx.app.log(TAG,
			"CREATE_WINDOW: Added graphics for handle " + newWindowHandle + " to map. Map size now: " + windowGraphicsMap.size());
		Gdx.app.log(TAG, "Stored new VulkanGraphics instance in map.");

		VulkanWindow newWindow = new VulkanWindow(listener, lifecycleListeners, config, // Use the specific config passed to
																													// newWindow
			this, newSurfaceHandle, newGraphicsInstance // Pass the NEW graphics instance
		);
	...
	/** Helper method to convert a GLFW monitor handle to a VulkanMonitor object. (Could also reside in
	 * VulkanApplicationConfiguration).
	 *
	...
	public Graphics.Monitor[] getMonitors () {
		...
		/*
		 * PointerBuffer glfwMonitors = GLFW.glfwGetMonitors(); if (glfwMonitors == null) return new Graphics.Monitor[0];
		 * Graphics.Monitor[] monitors = new Graphics.Monitor[glfwMonitors.limit()]; for (int i = 0; i < glfwMonitors.limit(); i++)
		 * { monitors[i] = toVulkanMonitor(glfwMonitors.get(i)); // Use helper } return monitors;
		 */
	}
	...
	public boolean setFullscreenMode (Graphics.DisplayMode displayMode) {
		...
		if (isFullscreen()) { // Check based on primary window handle
			...
			VulkanGraphics.VulkanDisplayMode currentMode = (VulkanGraphics.VulkanDisplayMode)getDisplayMode(getMonitor()); // Get
																																								// current
																																								// mode
																																								// for
																																								// primary
			...
			if (currentMode.monitorHandle == newMode.monitorHandle && currentMode.refreshRate == newMode.refreshRate) {
				...
				GLFW.glfwSetWindowMonitor(primaryWindowHandle, newMode.monitorHandle, 0, 0, newMode.width, newMode.height,
					newMode.refreshRate);
			}
		} else {
	...
	/** Updates the cached graphics dimensions. Call this after a resize or mode change when the new dimensions are known.
	 *
	...
	public void updateFramebufferInfo (int backBufferWidth, int backBufferHeight, int logicalWidth, int logicalHeight) {
		...
		Gdx.app.log(TAG, "updateFramebufferInfo called with: BB=" + backBufferWidth + " x " + backBufferHeight + ", Logical="
		...
		if (appConfig != null) {
			bufferFormat = new Graphics.BufferFormat(appConfig.r, appConfig.g, appConfig.b, appConfig.a, appConfig.depth,
				appConfig.stencil, appConfig.samples, false);
		} else {
			Gdx.app.error(TAG, "Config is null during updateFramebufferInfo, cannot update bufferFormat.");
		}

		Gdx.app.log(TAG, "Cached dimensions updated: BB=" + this.backBufferWidth + " x " + this.backBufferHeight + ", Logical="
	...
	/** Updates the application's reference to the currently active window. This is typically called by the main loop before
	 * updating/rendering a window, and potentially by a window during listener initialization if the listener needs to interact
	 * with app-level methods that depend on the current window context.
	 *
