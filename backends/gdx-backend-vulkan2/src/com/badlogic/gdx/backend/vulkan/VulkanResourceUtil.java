...
public class VulkanResourceUtil {
	...
	public static VulkanBuffer createManagedBuffer (long allocator, long size, int bufferUsageFlags, int vmaMemoryUsage,
		...
		try (MemoryStack stack = stackPush()) {
			// Define buffer creation parameters
			VkBufferCreateInfo bufferInfo = VkBufferCreateInfo.calloc(stack)
					.sType$Default()
					.size(size)
					.usage(bufferUsageFlags)
					.sharingMode(VK_SHARING_MODE_EXCLUSIVE); // Assume buffer is not shared between queue families

			// Define VMA allocation parameters
			VmaAllocationCreateInfo allocInfo = VmaAllocationCreateInfo.calloc(stack)
					.usage(vmaMemoryUsage) // Memory usage hint
					.flags(vmaAllocFlags); // Allocation strategy flags

			// Pointers to receive the created handles
			LongBuffer pBuffer = stack.mallocLong(1);
			PointerBuffer pAllocation = stack.mallocPointer(1);

			// Create the buffer and allocate memory using VMA
			vkCheck(vmaCreateBuffer(allocator, bufferInfo, allocInfo, pBuffer, pAllocation, null),
					"VMA failed to create buffer");

			long bufferHandle = pBuffer.get(0);
			long allocationHandle = pAllocation.get(0);

			// Basic validation of returned handles
			if (bufferHandle == VK_NULL_HANDLE || allocationHandle == VK_NULL_HANDLE) {
				// Attempt cleanup if partially created (shouldn't happen if vkCheck passes)
				if (bufferHandle != VK_NULL_HANDLE || allocationHandle != VK_NULL_HANDLE) {
	...
	public static void updateBuffer(long allocator, VulkanBuffer buffer, float[] data, long offset, long length) {
		...
		try {
			...
			if (mapped) {
				try { Vma.vmaUnmapMemory(allocator, buffer.allocationHandle); } catch (Exception unmapE) {
