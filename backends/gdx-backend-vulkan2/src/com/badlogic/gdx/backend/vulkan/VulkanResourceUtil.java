...
public class VulkanResourceUtil { // Example utility class
	...
	public static VulkanBuffer createManagedBuffer (long allocator, long size, int bufferUsageFlags, int vmaMemoryUsage,
		int vmaAllocFlags) {
		try (MemoryStack stack = stackPush()) {
			VkBufferCreateInfo bufferInfo = VkBufferCreateInfo.calloc(stack).sType$Default().size(size).usage(bufferUsageFlags)
				.sharingMode(VK_SHARING_MODE_EXCLUSIVE); // Assuming exclusive access for simplicity

			VmaAllocationCreateInfo allocInfo = VmaAllocationCreateInfo.calloc(stack).usage(vmaMemoryUsage) // VMA hint for memory
																																			// type/location
	...
	/** Creates a Vulkan image using VMA. (Simplified - add parameters for tiling, layers, mip levels etc. as needed)
	 *
	 * @param allocator VMA allocator instance handle.
	...
	public static VulkanImage createManagedImage (long allocator, int width, int height, int format, int tiling,
		...
		try (MemoryStack stack = stackPush()) {
			VkImageCreateInfo imageInfo = VkImageCreateInfo.calloc(stack).sType$Default().imageType(VK_IMAGE_TYPE_2D).format(format)
				.extent(e -> e.width(width).height(height).depth(1)).mipLevels(1) // TODO: Parameterize
				.arrayLayers(1) // TODO: Parameterize
				.samples(VK_SAMPLE_COUNT_1_BIT) // TODO: Parameterize for MSAA
				.tiling(tiling).usage(imageUsageFlags).sharingMode(VK_SHARING_MODE_EXCLUSIVE)
				.initialLayout(VK_IMAGE_LAYOUT_UNDEFINED); // Will need transition

			VmaAllocationCreateInfo allocInfo = VmaAllocationCreateInfo.calloc(stack).usage(vmaMemoryUsage).flags(vmaAllocFlags);
			// Use VMA_ALLOCATION_CREATE_DEDICATED_MEMORY_BIT in flags if required (check vkGetImageMemoryRequirements2)

			LongBuffer pImage = stack.mallocLong(1);
			PointerBuffer pAllocation = stack.mallocPointer(1);

			vkCheck(vmaCreateImage(allocator, imageInfo, allocInfo, pImage, pAllocation, null), "VMA failed to create image");
