...
public class VulkanGraphics extends AbstractGraphics implements Disposable {
	...
	/*
	 * public VulkanGraphics(long primaryWindowHandle, VulkanApplicationConfiguration config, VulkanDevice device, long
	 * vmaAllocatorHandle) { this.primaryWindowHandle = primaryWindowHandle; // Store handle of the first window this.config =
	 * config; this.app = (VulkanApplication) Gdx.app; // Get app reference this.vulkanDevice = device; this.vmaAllocator =
	 * vmaAllocatorHandle;
	 * 
	 * if (this.vulkanDevice == null || this.vmaAllocator == VK_NULL_HANDLE) { throw new
	 * GdxRuntimeException("VulkanDevice or VMA Allocator handle cannot be null for VulkanGraphics"); }
	 * 
	 * this.pipelineManager = new VulkanPipelineManager(this.vulkanDevice); this.descriptorManager = new
	 * VulkanDescriptorManager(this.vulkanDevice.getRawDevice());
	 * 
	 * int initialBackBufferWidth; int initialBackBufferHeight; int initialLogicalWidth; int initialLogicalHeight;
	 * 
	 * try (MemoryStack stack = MemoryStack.stackPush()) { IntBuffer pWidth = stack.mallocInt(1); IntBuffer pHeight =
	 * stack.mallocInt(1);
	 * 
	 * GLFW.glfwGetFramebufferSize(this.primaryWindowHandle, pWidth, pHeight); initialBackBufferWidth = pWidth.get(0);
	 * initialBackBufferHeight = pHeight.get(0);
	 * 
	 * pWidth.clear(); pHeight.clear();
	 * 
	 * GLFW.glfwGetWindowSize(this.primaryWindowHandle, pWidth, pHeight); initialLogicalWidth = pWidth.get(0); initialLogicalHeight
	 * = pHeight.get(0); } // MemoryStack automatically frees pWidth, pHeight
	 * 
	 * updateFramebufferInfo(initialBackBufferWidth, initialBackBufferHeight, initialLogicalWidth, initialLogicalHeight); }
	 */

	public VulkanGraphics (long windowHandle, VulkanApplicationConfiguration config, VulkanApplication app, VulkanDevice device,
		long vmaAllocatorHandle, VulkanPipelineManager pipelineManager, VulkanDescriptorManager descriptorManager) {
