...
// VkExtent2D.create() might be safer if it's stored long-term. Let's store a copy in createSwapchain instead.
	...
	private void createImageViews (MemoryStack stack) {
		...
		for (long image : swapchainImages) {
			...
			VkMemoryUtil.vkCheck(vkCreateImageView(rawDevice, createInfo, null, pImageView), "Failed to create image view");
	...
	private void createRenderPass (MemoryStack stack) {
		...
		VkMemoryUtil.vkCheck(vkCreateRenderPass(rawDevice, renderPassInfo, null, pRenderPass), "Failed to create render pass");
	...
	private void createCommandBuffers (MemoryStack stack) {
		...
		VkCommandBufferAllocateInfo allocInfo = VkCommandBufferAllocateInfo.calloc(stack)
			.sType(VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO).commandPool(vkDevice.getCommandPool()) // Use pool from VkDevice
		...
		VkMemoryUtil.vkCheck(vkAllocateCommandBuffers(rawDevice, allocInfo, pCommandBuffers), "Failed to allocate command buffers");
	...
	private void createSyncObjects (MemoryStack stack) {
		...
		VkMemoryUtil.vkCheck(vkCreateFence(rawDevice, fenceInfo, null, pFence), "Failed to create in-flight fence");
