...
public class VulkanGraphics extends AbstractGraphics implements Disposable {
	...
	public void initializeSwapchainAndResources () {
		...
		try (MemoryStack stack = stackPush()) {
			...
			/*
			 * Gdx.app.log(logTag, "Creating quad mesh..."); VertexAttributes quadAttributes = new VertexAttributes( new
			 * VertexAttribute(VertexAttributes.Usage.Position, 2, ShaderProgram.POSITION_ATTRIBUTE), new
			 * VertexAttribute(VertexAttributes.Usage.ColorUnpacked, 3, ShaderProgram.COLOR_ATTRIBUTE), new
			 * VertexAttribute(VertexAttributes.Usage.TextureCoordinates, 2, ShaderProgram.TEXCOORD_ATTRIBUTE + "0") ); this.quadMesh
			 * = new VulkanMesh(this.vulkanDevice, this.vmaAllocator, true, // isStatic = true quadVertices.length / 7, //
			 * numVertices = 4 quadIndices.length, // numIndices = 6 quadAttributes); this.quadMesh.setVertices(quadVertices); //
			 * Upload data this.quadMesh.setIndices(quadIndices); // Upload data Gdx.app.log(logTag,
			 * "Quad mesh created successfully.");
			 */
	...
	/** Gets the command buffer currently being recorded for this frame's rendering. This is only valid between successful calls to
	 * internal beginFrame() and endFrame() methods within the main render loop. Returns null otherwise.
	 * <p>
	 * Primarily intended for use by the ApplicationListener/Screen's render method to pass the command buffer to Vulkan-aware
	 * rendering components like VulkanSpriteBatch.
	 *
	...
	/** Gets the index of the swapchain image currently being rendered to. This is only valid between successful calls to internal
	 * beginFrame() and endFrame(). Returns -1 otherwise.
	 *
	...
	/** Gets the handle of the render pass currently active on the command buffer for this frame. This is typically the swapchain's
	 * main render pass, captured during beginFrame(). This is only valid between successful calls to internal beginFrame() and
	 * endFrame(). Returns VK_NULL_HANDLE otherwise.
	 *
	...
	private FrameInfo beginFrame () {
		...
		try (MemoryStack stack = stackPush()) {
			...
			try (MemoryStack stack2 = stackPush()) {
				...
				// Configure Viewport
				viewport.get(0).x(0.0f).y((float)extent.height()).width((float)extent.width()).height(-(float)extent.height()) // Positive
																																									// height
																																									// convention
				...
				scissor.get(0).offset(o -> o.set(0, 0)).extent(extent);
