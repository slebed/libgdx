...
public class VulkanGraphics extends AbstractGraphics implements Disposable {
	...
	public VulkanGraphics (long windowHandle, VulkanApplicationConfiguration config, VulkanDevice device,
		long vmaAllocatorHandle) {
	...
	public void initializeSwapchainAndResources () {
		...
		try (MemoryStack stack = stackPush()) {
			...
			this.vulkanSwapchain = new VulkanSwapchain.Builder().device(this.vulkanDevice).surface(this.surface)
				.windowHandle(this.windowHandle).configuration(this.config).build();
			...
			VulkanDescriptorManager.updateCombinedImageSampler(rawDevice, this.textureDescriptorSet, 0, // binding
			...
			if (renderPassHandle == VK_NULL_HANDLE)
				throw new GdxRuntimeException("Render pass handle invalid for pipeline creation");
			...
			pipelineManager.createDefaultPipeline(vertShaderFile, fragShaderFile, this.singleTextureLayoutHandle, renderPassHandle);
	...
	/** Cleans up the main command buffer List and potentially frees buffers if the pool wasn't created with
	 * RESET_COMMAND_BUFFER_BIT (though ours is). Primarily clears the Java list reference. */
	...
	/** Creates the command buffers used for recording rendering commands each frame. One command buffer is typically allocated per
	 * swapchain image. */
	private void createMainCommandBuffers (MemoryStack stack) {
		...
		VkCommandBufferAllocateInfo allocInfo = VkCommandBufferAllocateInfo.calloc(stack)
			.sType(VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO).commandPool(vulkanDevice.getCommandPool()) // Use main command
																																				// pool from
																																				// VulkanDevice
	...
	public boolean drawFrame () {
		...
		if (framebufferResized || (vulkanSwapchain != null && vulkanSwapchain.needsRecreation())) {
			// Gdx.app.log(logTag, "[DrawFrame] Recreation needed (resized=" + framebufferResized + ", needsRecreation=" +
			// (vulkanSwapchain != null && vulkanSwapchain.needsRecreation()) + ").");
		...
		try {
			...
			vulkanRenderer.bindDescriptorSet(pipelineManager.getPipelineLayout(), this.textureDescriptorSet, 0); // Bind the
																																					// texture's set
	...
	private void copyBuffer (long srcBuffer, long dstBuffer, long size) {
		...
		vulkanDevice.executeSingleTimeCommands(commandBuffer -> { // Pass lambda for recording
			try (MemoryStack stack = stackPush()) {
				VkBufferCopy.Buffer copyRegion = VkBufferCopy.calloc(1, stack).srcOffset(0) // Optional: if copying from specific
																														// offset
	...
	/*
	 * private void createDescriptorSetLayout() { Gdx.app.log(logTag, "Attempting to retrieve layout from DescriptorManager...");
	 * if (descriptorManager == null) { throw new GdxRuntimeException("DescriptorManager is null in createDescriptorSetLayout!"); }
	 * long layoutHandleFromManager = descriptorManager.getDefaultLayout(); Gdx.app.log(logTag,
	 * "Value returned by descriptorManager.getDefaultLayout(): " + layoutHandleFromManager); // <-- ADD LOG
	 * 
	 * this.singleTextureLayoutHandle = layoutHandleFromManager; // Assignment Gdx.app.log(logTag,
	 * "Value assigned to this.singleTextureLayoutHandle: " + this.singleTextureLayoutHandle); // <-- ADD LOG
	 * 
	 * // Keep the check if (this.singleTextureLayoutHandle == VK_NULL_HANDLE) { throw new
	 * GdxRuntimeException("Failed to get default layout from DescriptorManager (returned NULL)."); } Gdx.app.log(logTag,
	 * "Retrieved descriptor set layout from manager successfully."); }
	 * 
	 * private void createDescriptorSet() { // Allocate the set using the manager this.textureDescriptorSet =
	 * descriptorManager.allocateSet(); if (this.textureDescriptorSet == VK_NULL_HANDLE) { throw new
	 * GdxRuntimeException("Failed to allocate descriptor set from DescriptorManager"); } Gdx.app.log(logTag,
	 * "Allocated descriptor set from manager."); }
	 * 
	 * private void updateDescriptorSet() { // Use the static helper method
	 * 
	 * if (this.texture == null || this.textureDescriptorSet == VK_NULL_HANDLE) { throw new
	 * GdxRuntimeException("Cannot update descriptor set, texture or set handle is null."); }
	 * VulkanDescriptorManager.updateCombinedImageSampler( rawDevice, // Pass VkDevice this.textureDescriptorSet, 0, // binding
	 * this.texture // Pass the VulkanTexture object itself ); Gdx.app.log(logTag,
	 * "Updated descriptor set using manager helper for texture.");
	 * 
	 * }
	 */

	/** Handles start-of-frame Vulkan operations: waits for GPU, acquires swapchain image, resets fence, begins command buffer, and
	 * begins the render pass.
	 *
	 * @return A FrameInfo object containing success status, the active command buffer, and the swapchain image index. Returns
	 *         success=false if swapchain is out of date or acquire failed non-critically, requiring recreation check. */
	private FrameInfo beginFrame () {
		...
		try (MemoryStack stack = stackPush()) {
			...
			vkCheck(vkWaitForFences(rawDevice, fence, true, Long.MAX_VALUE), "vkWaitForFences failed!");
			...
			int acquireResult = vulkanSwapchain.acquireNextImage(imageAcquireSemaphore, VK_NULL_HANDLE, // No fence needed for
																																		// acquire
				pImageIndex);
			...
			if (acquireResult == VK_ERROR_OUT_OF_DATE_KHR) {
				...
				throw new GdxRuntimeException(
					"[beginFrame] Failed to acquire swap chain image! Result: " + VkResultDecoder.decode(acquireResult));
			}
			...
			if (commandBuffers == null || imageIndex < 0 || imageIndex >= commandBuffers.size()) {
				throw new GdxRuntimeException("[beginFrame] CommandBuffers list invalid or imageIndex (" + imageIndex
					+ ") out of bounds. CB Size=" + (commandBuffers != null ? commandBuffers.size() : "null"));
			}
	...
	/** Handles end-of-frame Vulkan operations: ends render pass, ends command buffer, submits command buffer to queue, and
	...
	private void endFrame (VkCommandBuffer commandBuffer, int imageIndex) {
		...
		try (MemoryStack stack = stackPush()) {
			...
			// --- 3. Presentation ---
			int presentResult = vulkanSwapchain.present(presentQueue, // Use present queue
				imageIndex, renderFinSem); // Wait for render finished semaphore from manager
			...
			if (presentResult != VK_SUCCESS && presentResult != VK_SUBOPTIMAL_KHR && presentResult != VK_ERROR_OUT_OF_DATE_KHR) {
				...
				throw new GdxRuntimeException(
					"[endFrame] Failed to present swap chain image! Result: " + VkResultDecoder.decode(presentResult));
			}
