...
public class VkDevice implements VkResource {
	...
	private VkDevice (VkDeviceHandle deviceHandle, VkQueue graphicsQueue, VkPhysicalDevice physicalDevice, long commandPool,
		...
		public VkDevice build () {
			try (MemoryStack stack = stackPush()) {
				...
				VkDeviceQueueCreateInfo.Buffer queueCreateInfo = VkDeviceQueueCreateInfo.calloc(1, stack)
					.sType(VK_STRUCTURE_TYPE_DEVICE_QUEUE_CREATE_INFO).queueFamilyIndex(queueFamilyIndex)
				...
				VkDeviceCreateInfo createInfo = VkDeviceCreateInfo.calloc(stack).sType(VK_STRUCTURE_TYPE_DEVICE_CREATE_INFO)
					.pQueueCreateInfos(queueCreateInfo).ppEnabledExtensionNames(requiredExtensions).pEnabledFeatures(null);
				...
				VkCommandPoolCreateInfo poolInfo = VkCommandPoolCreateInfo.calloc(stack)
					.sType(VK_STRUCTURE_TYPE_COMMAND_POOL_CREATE_INFO).queueFamilyIndex(queueFamilyIndex)
	...
	public VkCommandBuffer beginSingleTimeCommands () {
		try (MemoryStack stack = MemoryStack.stackPush()) {
			...
			VkCommandBufferAllocateInfo allocInfo = VkCommandBufferAllocateInfo.calloc(stack)
				.sType(VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO).level(VK_COMMAND_BUFFER_LEVEL_PRIMARY).commandPool(commandPool) // We'll
																																												// need
																																												// to
																																												// create
																																												// this
